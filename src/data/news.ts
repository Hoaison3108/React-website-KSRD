export const newsCategories = [
  'Tất cả',
  'Tin Công Ty',
  'Sự Kiện & Giải Thưởng',
  'Trách Nhiệm Xã Hội',
  'Sản Phẩm & Công Nghệ'
];

export const news = [
  {
    id: 1,
    title: 'Lễ khởi công nhà máy sản xuất gạch không nung công suất lớn',
    category: 'Tin Công Ty',
    date: '2025-08-15',
    displayDate: '15 TH08',
    image: 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Rạng Đông chính thức khởi công xây dựng dây chuyền sản xuất gạch không nung hiện đại nhất khu vực, đáp ứng nhu cầu ngày càng cao của thị trường xây dựng xanh.',
    content: [
      'Ngày 15/08/2023, Công ty Khoáng sản Rạng Đông đã long trọng tổ chức lễ khởi công xây dựng nhà máy sản xuất gạch không nung công nghệ cao tại Khu công nghiệp Hàm Kiệm, tỉnh Bình Thuận. Dự án có tổng vốn đầu tư lên đến 50 tỷ đồng, với công suất thiết kế đạt 50 triệu viên/năm.',
      'Phát biểu tại buổi lễ, ông Nguyễn Văn A - Tổng Giám đốc công ty nhấn mạnh: "Việc đầu tư vào dây chuyền sản xuất gạch không nung là bước đi chiến lược của Rạng Đông nhằm đón đầu xu hướng vật liệu xanh, thân thiện với môi trường, đồng thời thực hiện chủ trương của Chính phủ về việc hạn chế sử dụng gạch đất sét nung."',
      'Nhà máy được trang bị dây chuyền công nghệ hiện đại nhập khẩu từ Đức, tự động hóa hoàn toàn từ khâu phối trộn nguyên liệu đến khâu đóng gói thành phẩm. Sản phẩm gạch không nung của Rạng Đông không chỉ có cường độ chịu lực cao, khả năng cách âm, cách nhiệt tốt mà còn có mẫu mã đa dạng, đáp ứng các tiêu chuẩn khắt khe của các công trình xây dựng hiện đại.',
      'Dự kiến, nhà máy sẽ chính thức đi vào hoạt động vào quý 2 năm 2024, tạo việc làm cho hơn 100 lao động địa phương và đóng góp tích cực vào ngân sách nhà nước. Đây cũng là minh chứng cho cam kết phát triển bền vững của Rạng Đông, gắn liền lợi ích doanh nghiệp với trách nhiệm xã hội và bảo vệ môi trường.'
    ],
    detailImages: [
      'https://images.unsplash.com/photo-1531834685032-c34bf0d84c77?q=80&w=1000&auto=format&fit=crop',
      'https://images.unsplash.com/photo-1589939705384-5185137a7f0f?q=80&w=1000&auto=format&fit=crop'
    ]
  },
  {
    id: 2,
    title: 'Bê tông Rạng Đông đạt chuẩn ISO 9001:2015',
    category: 'Sự Kiện & Giải Thưởng',
    date: '2023-07-20',
    displayDate: '20 TH07',
    image: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Hệ thống quản lý chất lượng của chúng tôi đã được tổ chức quốc tế công nhận, khẳng định cam kết chất lượng vượt trội trong từng mẻ bê tông thương phẩm.',
    content: [
      'Sau hơn 6 tháng nỗ lực xây dựng và hoàn thiện hệ thống quản lý, ngày 20/07/2023, Công ty Khoáng sản Rạng Đông vinh dự đón nhận chứng chỉ ISO 9001:2015 do tổ chức chứng nhận quốc tế cấp. Đây là sự ghi nhận xứng đáng cho những nỗ lực không ngừng nghỉ của tập thể cán bộ công nhân viên công ty trong việc nâng cao chất lượng sản phẩm và dịch vụ.',
      'Tiêu chuẩn ISO 9001:2015 là phiên bản mới nhất của hệ thống quản lý chất lượng quốc tế, tập trung vào việc kiểm soát rủi ro và nâng cao sự hài lòng của khách hàng. Việc áp dụng thành công tiêu chuẩn này giúp Rạng Đông chuẩn hóa quy trình sản xuất, kiểm soát chặt chẽ chất lượng nguyên vật liệu đầu vào và sản phẩm đầu ra.',
      'Với việc đạt chuẩn ISO 9001:2015, Rạng Đông cam kết cung cấp ra thị trường các sản phẩm bê tông thương phẩm, gạch xây dựng có chất lượng ổn định, đồng nhất và đáp ứng mọi yêu cầu kỹ thuật của khách hàng. Đồng thời, công ty cũng không ngừng cải tiến quy trình, nâng cao năng lực quản lý để mang lại giá trị tốt nhất cho đối tác và khách hàng.',
      'Trong thời gian tới, Rạng Đông sẽ tiếp tục duy trì và cải tiến hệ thống quản lý chất lượng, đồng thời hướng tới việc áp dụng các tiêu chuẩn quản lý môi trường ISO 14001 và an toàn sức khỏe nghề nghiệp ISO 45001, khẳng định vị thế là đơn vị cung cấp vật liệu xây dựng hàng đầu tại khu vực.'
    ],
    detailImages: [
      'https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?q=80&w=1000&auto=format&fit=crop'
    ]
  },
  {
    id: 3,
    title: 'Hoạt động từ thiện: Xây cầu cho em tại vùng cao',
    category: 'Trách Nhiệm Xã Hội',
    date: '2023-06-05',
    displayDate: '05 TH06',
    image: 'https://images.unsplash.com/photo-1545139224-79b176937ee9?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Công ty đã tài trợ 100% kinh phí xây dựng cầu dân sinh tại xã vùng cao, giúp bà con đi lại thuận tiện và các em nhỏ đến trường an toàn hơn.',
    content: [
      'Hưởng ứng phong trào "Chung tay vì cộng đồng", ngày 05/06/2023, Công ty Khoáng sản Rạng Đông đã phối hợp với chính quyền địa phương tổ chức lễ khánh thành và bàn giao cầu dân sinh tại xã La Dạ, huyện Hàm Thuận Bắc. Cây cầu được xây dựng kiên cố bằng bê tông cốt thép, thay thế cho cây cầu gỗ tạm bợ, xuống cấp nghiêm trọng trước đây.',
      'Công trình có tổng chiều dài 20m, rộng 3.5m, với tổng kinh phí xây dựng hơn 500 triệu đồng do Công ty Rạng Đông tài trợ hoàn toàn. Việc hoàn thành cây cầu không chỉ giúp bà con nhân dân đi lại, vận chuyển nông sản thuận tiện mà còn đảm bảo an toàn cho hàng trăm em học sinh đến trường, đặc biệt là trong mùa mưa lũ.',
      'Tại buổi lễ, đại diện lãnh đạo xã đã bày tỏ lòng cảm ơn sâu sắc đến Ban lãnh đạo và toàn thể nhân viên Công ty Rạng Đông. Cây cầu mới là món quà ý nghĩa, thiết thực, góp phần thay đổi diện mạo nông thôn và thúc đẩy phát triển kinh tế - xã hội của địa phương.',
      'Đây là một trong chuỗi các hoạt động an sinh xã hội thường niên của Rạng Đông. Bên cạnh hoạt động sản xuất kinh doanh, công ty luôn chú trọng đến trách nhiệm xã hội, tích cực tham gia các hoạt động từ thiện, đền ơn đáp nghĩa, khuyến học khuyến tài, góp phần xây dựng cộng đồng ngày càng tốt đẹp hơn.'
    ],
    detailImages: [
      'https://images.unsplash.com/photo-1503387762-592deb58ef4e?q=80&w=1000&auto=format&fit=crop',
      'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?q=80&w=1000&auto=format&fit=crop'
    ]
  },
  {
    id: 4,
    title: 'Hội thảo: Xu hướng vật liệu xây dựng xanh 2024',
    category: 'Sự Kiện & Giải Thưởng',
    date: '2023-11-10',
    displayDate: '10 TH11',
    image: 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Tham gia hội thảo chuyên đề về vật liệu xây dựng bền vững, Rạng Đông chia sẻ tầm nhìn về phát triển các sản phẩm thân thiện với môi trường.'
  },
  {
    id: 5,
    title: 'Ký kết hợp tác chiến lược với Tập đoàn Xây dựng Hòa Bình',
    category: 'Tin Công Ty',
    date: '2023-10-25',
    displayDate: '25 TH10',
    image: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Sự kiện đánh dấu bước ngoặt quan trọng trong việc mở rộng thị trường và nâng cao năng lực cung ứng cho các dự án quy mô lớn.'
  },
  {
    id: 6,
    title: 'Ra mắt dòng sản phẩm gạch Terrazzo cao cấp mới',
    category: 'Sản Phẩm & Công Nghệ',
    date: '2023-09-12',
    displayDate: '12 TH09',
    image: 'https://images.unsplash.com/photo-1620626012053-1c167f7ebc8d?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Dòng sản phẩm mới với độ bền vượt trội và mẫu mã đa dạng, hứa hẹn sẽ là lựa chọn hàng đầu cho các công trình cảnh quan đô thị.'
  },
  {
    id: 7,
    title: 'Đào tạo an toàn lao động định kỳ quý 3/2023',
    category: 'Tin Công Ty',
    date: '2023-09-05',
    displayDate: '05 TH09',
    image: 'https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Nâng cao ý thức và kỹ năng an toàn cho toàn thể cán bộ công nhân viên là ưu tiên hàng đầu tại các nhà máy sản xuất của Rạng Đông.'
  },
  {
    id: 8,
    title: 'Triển lãm Vietbuild 2023: Gian hàng Rạng Đông thu hút khách tham quan',
    category: 'Sự Kiện & Giải Thưởng',
    date: '2023-08-28',
    displayDate: '28 TH08',
    image: 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Tại triển lãm quốc tế Vietbuild, gian hàng của chúng tôi đã đón tiếp hàng ngàn lượt khách tham quan và nhận được nhiều phản hồi tích cực.'
  },
  {
    id: 9,
    title: 'Chương trình "Mái ấm công đoàn" trao tặng nhà tình thương',
    category: 'Trách Nhiệm Xã Hội',
    date: '2023-07-15',
    displayDate: '15 TH07',
    image: 'https://images.unsplash.com/photo-1589939705384-5185137a7f0f?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Công đoàn công ty đã trao tặng 2 căn nhà tình thương cho công nhân có hoàn cảnh khó khăn, thể hiện tinh thần tương thân tương ái.'
  },
  {
    id: 10,
    title: 'Nâng cấp hệ thống trạm trộn bê tông tại Bình Thuận',
    category: 'Sản Phẩm & Công Nghệ',
    date: '2023-12-01',
    displayDate: '01 TH12',
    image: 'https://images.unsplash.com/photo-1581094794329-c8112a89af12?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Việc nâng cấp công nghệ giúp tăng công suất trạm trộn lên 120m3/h, đảm bảo tiến độ cung ứng cho các dự án trọng điểm trong khu vực.'
  },
  {
    id: 11,
    title: 'Rạng Đông đạt giải thưởng Doanh nghiệp tiêu biểu 2024',
    category: 'Sự Kiện & Giải Thưởng',
    date: '2024-01-15',
    displayDate: '15 TH01',
    image: 'https://images.unsplash.com/photo-1541888946425-d81bb19240f5?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Ghi nhận những đóng góp tích cực vào sự phát triển kinh tế địa phương và các hoạt động xã hội, Rạng Đông vinh dự nhận giải thưởng danh giá.',
    content: [
      'Ngày 15/01/2024, tại Trung tâm Hội nghị tỉnh Bình Thuận, Công ty Khoáng sản Rạng Đông đã vinh dự được trao tặng danh hiệu "Doanh nghiệp tiêu biểu tỉnh Bình Thuận năm 2024". Giải thưởng nhằm tôn vinh các doanh nghiệp có thành tích xuất sắc trong hoạt động sản xuất kinh doanh, chấp hành tốt pháp luật và đóng góp tích cực cho cộng đồng.',
      'Đây là sự ghi nhận xứng đáng cho những nỗ lực không ngừng nghỉ của Ban lãnh đạo và toàn thể cán bộ công nhân viên công ty trong bối cảnh kinh tế còn nhiều khó khăn, thách thức. Rạng Đông đã duy trì tốc độ tăng trưởng ổn định, đảm bảo việc làm và thu nhập cho người lao động, đồng thời tích cực tham gia các hoạt động an sinh xã hội.',
      'Phát biểu tại lễ trao giải, đại diện công ty chia sẻ: "Giải thưởng này là động lực to lớn để chúng tôi tiếp tục phấn đấu, đổi mới sáng tạo, nâng cao chất lượng sản phẩm dịch vụ, hướng tới mục tiêu phát triển bền vững và đóng góp nhiều hơn nữa cho sự phát triển chung của tỉnh nhà."'
    ]
  },
  {
    id: 12,
    title: 'Triển khai công nghệ sản xuất xanh giảm thiểu khí thải',
    category: 'Sản Phẩm & Công Nghệ',
    date: '2024-02-20',
    displayDate: '20 TH02',
    image: 'https://images.unsplash.com/photo-1503387762-592deb58ef4e?q=80&w=1000&auto=format&fit=crop',
    excerpt: 'Rạng Đông tiên phong áp dụng công nghệ thu hồi nhiệt và lọc bụi tĩnh điện trong dây chuyền sản xuất, hướng tới mục tiêu Net Zero.',
    content: [
      'Nhằm thực hiện cam kết bảo vệ môi trường và phát triển bền vững, đầu năm 2024, Công ty Khoáng sản Rạng Đông đã chính thức đưa vào vận hành hệ thống xử lý khí thải và thu hồi nhiệt dư tại nhà máy sản xuất vật liệu xây dựng.',
      'Hệ thống mới sử dụng công nghệ lọc bụi tĩnh điện tiên tiến, giúp loại bỏ đến 99% bụi mịn phát sinh trong quá trình sản xuất. Đồng thời, hệ thống thu hồi nhiệt dư giúp tận dụng nhiệt lượng tỏa ra để sấy khô nguyên liệu, giúp tiết kiệm nhiên liệu và giảm phát thải khí nhà kính.',
      'Việc đầu tư vào công nghệ xanh không chỉ giúp Rạng Đông tuân thủ nghiêm ngặt các quy định về bảo vệ môi trường mà còn nâng cao hiệu quả sản xuất, giảm chi phí vận hành trong dài hạn. Đây là bước đi cụ thể hóa chiến lược phát triển bền vững, hướng tới mục tiêu sản xuất xanh - sạch - đẹp.'
    ]
  }
];
